...
contract Cryptos is ERC20Interface{
    ...
    //The keyword virtual means that the function can change its behavior in the derived contracts by overwriting,
...
contract CryptosICO is Cryptos{
    ...
    uint public saleStart = block.timestamp; //you can add how many second from start time
    uint public saleEnd = block.timestamp + 604800; //one week(number of seconds in a week)
    ...
    constructor(address payable _deposit){
        deposit = _deposit; //a deposit address (EOA account) that automatically receives the ETH sent to the ICO contract; 
        admin = msg.sender; 
    ...
    // can be checked on FrontEnd
    event Invest(address investor, uint value, uint tokens);
    
    
    // function called when sending eth to the contract
    // This will be called when somebody sends ETH to the contract and receives cryptos in turn.
